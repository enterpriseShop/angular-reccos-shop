...
export class NotificationsDrawerComponent {
  ...
  getBadgeBg(type: string): string {
    switch (type) {
      case 'warning': return 'bg-amber-100 dark:bg-amber-950/60 text-[#E9B949]';
      case 'success': return 'bg-emerald-100 dark:bg-emerald-950/60 text-[#5BAE6A]';
      case 'error': return 'bg-rose-100 dark:bg-rose-950/60 text-[#D66A6A]';
      case 'info': default: return 'bg-blue-100 dark:bg-blue-950/60 text-[#5A8DEE]';
    }
  }
}
